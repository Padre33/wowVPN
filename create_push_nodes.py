...
for ip in NODES:
    ...
    try:
        ...
        sftp.close()
        # Optionally reload the server instead of full restart if aivpn supports hot-reload
        # But for now a PUD/Pkill is safest. (Wait, if I restart it, all users disconnect!)
        # Does aivpn-server auto-watch clients.json? YES! Rust core has hot-reload!
        ssh.close()
        print(f"Synced clients.json to {ip}")
    except Exception as e:
