...
/**
 * Pure-Kotlin BLAKE3 implementation (single-chunk, â‰¤1024 bytes input).
 *
 * Supports:
 *  - deriveKey(context, material) â€“ key derivation mode
 *  - keyedHash(key, data)         â€“ keyed hash mode
 *
 * This minimal implementation handles only inputs that fit in a single BLAKE3 chunk
 * (up to 1024 bytes), which is sufficient for the ShadeVPN protocol.
 */
object Blake3 {
    ...
    fun deriveKey(context: String, material: ByteArray): ByteArray {
        // Step 1: Hash context string with DERIVE_KEY_CONTEXT flag â†’ context key
    ...
    // â”€â”€ Byte â†” word conversions (little-endian) â”€â”€
