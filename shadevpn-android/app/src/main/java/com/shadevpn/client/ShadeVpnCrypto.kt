...
/**
 * ShadeVPN protocol crypto engine (pure Kotlin, no JNI).
 *
 * Implements a simplified version of the ShadeVPN wire protocol:
 ...
 */
class ShadeVPNCrypto(private val serverStaticPub: ByteArray, private val psk: ByteArray? = null) {
    ...
    fun buildInitPacket(): ByteArray {
        ...
        val mask = Blake3.deriveKey("ShadeVPN-eph-obfuscation-v1", serverStaticPub)
        ...
        // Build ShadeVPN packet with eph_pub included
    ...
    /**
     * Process ServerHello â€” complete the PFS ratchet.
    ...
    /**
     * Encrypt an outbound IP packet into the ShadeVPN wire format.
     */
    @Synchronized
    ...
    /**
     * Decrypt an inbound ShadeVPN packet and return the inner IP packet, or null.
     */
    @Synchronized
    fun decryptDataPacket(packet: ByteArray): ByteArray? {
        ...
        return when (innerType) {
            0x01 -> inner.copyOfRange(INNER_HEADER_SIZE, inner.size) // Data â†’ IP packet
            else -> null // Control packets handled separately
        }
    }

    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Sliding-window anti-replay â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    private fun deriveKeys(sharedSecret: ByteArray, clientPub: ByteArray, psk: ByteArray? = null) {
        ...
        sessionKey = Blake3.deriveKey("ShadeVPN-session-key-v1", input)
        tagSecret = Blake3.deriveKey("ShadeVPN-tag-secret-v1", input)
    }

    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ ChaCha20-Poly1305 via Android API â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ X25519 scalar multiplication (BigInteger) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
