...
/**
 * Android VPN service â€” thin orchestrator over the Rust core (libShadeVPN_core.so).
 *
 ...
 *   - VpnService.protect() â€” called from inside Rust via JNI on this instance
...
class ShadeVpnService : VpnService() {
    ...
    companion object {
        const val ACTION_CONNECT    = "com.ShadeVPN.CONNECT"
        const val ACTION_DISCONNECT = "com.ShadeVPN.DISCONNECT"
        private const val CHANNEL_ID      = "ShadeVPN_vpn"
    ...
    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Service lifecycle â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            try {
                while (isActive && !manualDisconnect) {
                    try {
                        ...
                        runTunnel()
                        // runTunnel() returns normally only on Rust rekey trigger â€” reconnect fast.
    ...
    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Tunnel session â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    private suspend fun runTunnel() {
        ...
        for (pkg in allowedApps) {
            try {
                ...
                // Package may have been uninstalled â€” skip silently
            }
        }
        ...
        // Setting null allows automatic network selection and seamless WiFiâ†”cellular switching.
        ...
        sessionEstablished = false
        isRunning          = true
        sessionId++     // new session â€” invalidates any queued upgradePendingJob
        ...
        val statsJob = serviceScope.launch {
            while (isActive) {
                delay(1_000L)
                trafficCallback?.invoke(ShadeVpnJni.getUploadBytes(), ShadeVpnJni.getDownloadBytes())
            }
        }
    ...
    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Network callbacks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    *   - The Rust side has an aggressive RX silence detector as backup
        ...
        val callback = object : ConnectivityManager.NetworkCallback() {
            ...
            override fun onLost(network: Network) {
                ...
                if (!hasUsableDefault && isRunning) {
                    ...
                    if (now - lastNetworkEventAtMs >= NETWORK_EVENT_DEBOUNCE_MS) {
                        lastNetworkEventAtMs = now
                        Log.d(TAG, "No usable underlying network â€” stopping tunnel for fast reconnect")
                        networkTrigger = true
                        ShadeVpnJni.stopTunnel()
                    }
                }
    ...
    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Stop â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    override fun onRevoke() {
        Log.w(TAG, "onRevoke() â€” signalling Rust to exit, reconnect loop will restart")
        ShadeVpnJni.stopTunnel()
        // Do NOT call super.onRevoke() â€” it calls stopSelf() which bypasses reconnect.
    }
    ...
    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Address parsing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
